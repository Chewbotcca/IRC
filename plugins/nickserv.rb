class NickServ
  ...
  def register(m, pass, email)
    if authenticate(m) && checkperm(m, m.user.name, 'nickserv')
      ...
      m.reply "I sent verification to nickserv. It might need to verify the email. In that case, type `#{CONFIG['prefix']}nsverify [code]` to verify it. Otherwise, the password has been logged into the config and it will auto-authenticate on startup!"
    else
